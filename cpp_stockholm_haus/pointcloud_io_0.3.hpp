                ...
                uint8_t alpha_pct   = 180)
{
    ...
    for (int pi = 0; pi < (int)planes.size(); ++pi) {
        RGB c = plane_color(pi);
        for (uint32_t idx : planes[pi].inliers)
    ...
    for (int pi = 0; pi < (int)planes.size(); ++pi) {
        ...
        if (pd.inliers.size() < 3) continue;

        std::array<float,3> u, v;
        detail::plane_basis(pd.normal, u, v);

        // Project inliers onto plane's 2-D coordinate system
        std::vector<std::array<float,2>> pts2d;
        pts2d.reserve(pd.inliers.size());
        ...
        float inv = 1.f / (float)pd.inliers.size();
