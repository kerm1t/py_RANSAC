...
Usage:
  ...
  # Segment + show one normal arrow per plane
  python visualize_planes.py scan.pcd --segment --show-normals

  # Control arrow length (default: 20% of plane diagonal)
  python visualize_planes.py scan.pcd --segment --show-normals --normal-scale 0.5

  # Show per-point normals on a plain cloud (estimated on the fly)
...
def _rotation_matrix_to_vec(target: np.ndarray) -> np.ndarray:
    """Return 3×3 rotation matrix that maps +Z → target (unit vector)."""
    target = target / np.linalg.norm(target)
    z = np.array([0.0, 0.0, 1.0])
    axis = np.cross(z, target)
    sin_a = np.linalg.norm(axis)
    cos_a = float(np.dot(z, target))
    if sin_a < 1e-6:
...
def make_normal_arrows(plane_info: list,
    ...
    """
    Build one arrow mesh per plane, centred on the inlier centroid,
    pointing along the plane normal.

    Parameters
    ----------
    plane_info   : list of (plane_model, inlier_pts_np) from segment_planes()
    normal_scale : arrow length in metres.  If None, auto-sizes to 20% of
                   the plane's bounding-box diagonal so it always looks
                   proportional regardless of scene scale.

    Returns a list of o3d.geometry.TriangleMesh (one per plane).
    """
    arrows = []
    for pi, (plane_model, inlier_pts) in enumerate(plane_info):
        ...
        # Auto-scale: 20 % of the inlier bounding-box diagonal
        if normal_scale is None:
            bbox_diag = np.linalg.norm(inlier_pts.max(axis=0) -
                                       inlier_pts.min(axis=0))
            length = max(bbox_diag * 0.20, 0.05)   # floor at 5 cm
        else:
            length = float(normal_scale)

        arrow = o3d.geometry.TriangleMesh.create_arrow(
            cylinder_radius = length * 0.035,
            cone_radius     = length * 0.070,
            cylinder_height = length * 0.75,
            cone_height     = length * 0.25,
            resolution      = 20)

        # Rotate +Z → normal, translate to centroid
        R = _rotation_matrix_to_vec(normal)
        T = np.eye(4)
        T[:3, :3] = R
        T[:3,  3] = centroid
        arrow.transform(T)
        ...
        print(f"  Plane {pi} normal: ({a:.3f}, {b:.3f}, {c:.3f})  "
              f"arrow length={length:.3f} m")
...
def visualize(path, segment, num_planes, threshold, save_mesh,
    ...
    if has_faces and not segment:
        ...
        if not segment:
            ...
            # Per-point normals on a plain cloud
            if show_normals:
                print("Estimating per-point normals …")
            ...
            # Normal arrows — one per plane from RANSAC normals
            if show_normals:
                print("\nBuilding normal arrows …")
    ...
    # point_show_normal renders small per-point sticks when normals are present
    show_pt_normals = show_normals and not segment

    print("\nOpening viewer  [q / Esc = quit]\n"
          "  Left drag — rotate  |  Right drag — pan  |  Scroll — zoom  |  R — reset\n")
    o3d.visualization.draw_geometries(
        geometries,
        window_name="RANSAC plane viewer",
        width=1280, height=720,
        point_show_normal=show_pt_normals)
...
def main():
    ...
    ap.add_argument("--show-normals",  action="store_true",
                    help="Show normal arrows (one per plane when --segment, "
