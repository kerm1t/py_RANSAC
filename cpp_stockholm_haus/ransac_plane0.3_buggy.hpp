                         ...
                         float axis_snap_deg = 45.f)
{
    ...
    // Find the plane with the most inliers that got assigned
    int dominant = -1;
    for (int pi = 0; pi < (int)result.planes.size(); ++pi) {
    ...
    // Translate along the assigned axis so d → 0
    int   ax  = result.axis_assignment[dominant];
    float off = result.planes[dominant].d; // dot(n,p)+d=0 → shift by d along n
    // Shift every point by -d along the axis direction
    Point3f shift{};
    shift[ax] = -off; // plane normal after alignment is exactly ±axis, so d = -mean_coord
    printf("Translating by (%.3f, %.3f, %.3f) to put plane %d at origin\n",
           shift[0], shift[1], shift[2], dominant);
