...
def grid_filter(pts2d: np.ndarray,
    ...
    Bins points into a regular grid in UV space and keeps only those
    whose cell contains at least `min_pts` points.

    Parameters
    ----------
    pts2d     : (N, 2) array of UV coordinates
    cell_size : grid cell side length (same units as the point cloud)
    min_pts   : minimum occupancy to keep a cell
    ...
    if nu * nv > 4_000_000:
        return np.ones(len(pts2d), dtype=bool)
...
def build_plane_mesh(plane_model, inlier_pts, color,
    ...
    # Grid filter
    mask = grid_filter(pts2d, cell_size=cell_size, min_pts=min_pts)
...
def save_plane_mesh_py(out_path, pcd, plane_info, alpha=180):
...
def main():
    ...
    args = ap.parse_args()
    cell  = 1e9 if args.no_grid else args.cell_size   # huge cell = no filter
    visualize(args.file, args.segment, args.num_planes,
              args.threshold, args.save_mesh, cell, args.min_pts)
