...
// ─────────────────────────────────────────────────────────────────────
// Offset plane rectangles
//
// For each detected plane, generates one or more axis-aligned rectangular
// quads that are parallel to the plane's convex hull and offset from it
// by ±distance along the plane normal.
//
// The rectangle is the axis-aligned bounding box of the hull projected
// into the plane's UV space — slightly larger than the hull itself, which
// gives a clean "clearance slab" appearance in viewers.
//
// Both sides of each plane can be written (e.g. +d and -d for a wall
// with inner and outer offsets), or a single side.
//
// Usage:
//   pcio::OffsetConfig off;
//   off.distances     = {0.05f, 0.10f};   // two offset shells
//   off.both_sides    = true;              // ±distance
...
inline MeshStats
save_offset_planes(const std::string&            path,
                   const std::vector<Point3f>&   pts,
                   const std::vector<PlaneDesc>& planes,
                   const OffsetConfig&           cfg = {})
{
    MeshStats stats{};
    if (planes.empty() || cfg.distances.empty()) return stats;

    // ── Collect all offset quads ──────────────────────────────────────
    struct OVert { float x,y,z; RGB col; };
    struct OTri  { uint32_t a,b,c; RGB col; };

    std::vector<OVert> verts;
    std::vector<OTri>  tris;
    ...
    for (int pi = 0; pi < (int)planes.size(); ++pi) {
        ...
        // Grid filter to get the relevant inlier subset
        std::vector<uint32_t> kept;
        if (cfg.grid.enabled && pd.inliers.size() >= 3) {
            kept = detail::grid_filter_inliers(
                pts, pd.inliers, pd.normal, u, v,
                cfg.grid.cell_size, cfg.grid.min_pts_per_cell,
                cfg.grid.erode_iters, cfg.grid.dilate_iters);
        } else {
            kept = pd.inliers;
        }
        if (kept.size() < 3) continue;

        // Compute inlier centroid, snapped to plane
        float cx=0,cy=0,cz=0;
        for (uint32_t idx : kept) {
            cx+=pts[idx][0]; cy+=pts[idx][1]; cz+=pts[idx][2];
        }
        float inv = 1.f / (float)kept.size();
        cx*=inv; cy*=inv; cz*=inv;
        float snap = pd.normal[0]*cx + pd.normal[1]*cy + pd.normal[2]*cz + pd.d;
        float pcx = cx - pd.normal[0]*snap;
        float pcy = cy - pd.normal[1]*snap;
        float pcz = cz - pd.normal[2]*snap;

        // UV centroid (for relative offsets)
        float pu_c = detail::dot3({pcx,pcy,pcz}, u);
        float pv_c = detail::dot3({pcx,pcy,pcz}, v);

        // UV bounding box of kept inliers
        float umin= 1e30f, umax=-1e30f, vmin_= 1e30f, vmax_=-1e30f;
        for (uint32_t idx : kept) {
            float pu = detail::dot3(pts[idx], u);
            float pv = detail::dot3(pts[idx], v);
            umin=std::min(umin,pu); umax=std::max(umax,pu);
        ...
        // Slightly desaturate offset planes so they're visually distinct
        // from the original hull (mix toward white by 30%)
        auto offset_col = [](RGB c, float t) -> RGB {
            return { (uint8_t)(c.r + t*(255-c.r)),
                     (uint8_t)(c.g + t*(255-c.g)),
                     (uint8_t)(c.b + t*(255-c.b)) };
        };

        // For each distance (and optionally both sides)
        int dist_idx = 0;
        for (float dist : cfg.distances) {
            // Vary desaturation by distance index so multiple shells differ
            float desat = 0.25f + 0.15f * dist_idx;
            RGB rcol = offset_col(col, std::min(desat, 0.7f));
            ++dist_idx;

            auto write_quad = [&](float signed_dist) {
                // Offset = centroid + signed_dist * normal
                float ox = pcx + signed_dist * pd.normal[0];
                float oy = pcy + signed_dist * pd.normal[1];
                float oz = pcz + signed_dist * pd.normal[2];

                uint32_t base = (uint32_t)verts.size();
                for (auto& c2 : corners) {
                    float du = c2[0], dv = c2[1];
                    verts.push_back({
                        ox + du*u[0] + dv*v[0],
                        oy + du*u[1] + dv*v[1],
                        oz + du*u[2] + dv*v[2],
                ...
                tris.push_back({base+0, base+1, base+2, rcol});
...
//            if (cfg.both_sides) write_quad(-dist);
    ...
    // ── Write PLY (vertices + faces only, no cloud points) ────────────
    size_t n_v = verts.size();
    size_t n_f = tris.size();
    ...
    std::string hdr =
        "ply\nformat binary_little_endian 1.0\n"
        "comment offset planes saved by pointcloud_io.hpp\n"
        "element vertex " + std::to_string(n_v) + "\n"
        "property float x\nproperty float y\nproperty float z\n"
        "property uchar red\nproperty uchar green\nproperty uchar blue\n"
        "element face " + std::to_string(n_f) + "\n"
        "property list uchar uint vertex_indices\n"
        "property uchar red\nproperty uchar green\nproperty uchar blue\n"
        "property uchar alpha\n"
        "end_header\n";
...
#pragma pack(push,1)
    struct VR { float x,y,z; uint8_t r,g,b; };
    struct FR { uint8_t cnt; uint32_t i0,i1,i2; uint8_t r,g,b,a; };
#pragma pack(pop)
    static_assert(sizeof(VR)==15,"");
    static_assert(sizeof(FR)==17,"");

    for (auto& ov : verts) {
        VR rec{ov.x,ov.y,ov.z, ov.col.r,ov.col.g,ov.col.b};
        f.write(reinterpret_cast<const char*>(&rec), sizeof(rec));
    }
    for (auto& ot : tris) {
        FR rec{3, ot.a,ot.b,ot.c, ot.col.r,ot.col.g,ot.col.b, cfg.alpha};
        f.write(reinterpret_cast<const char*>(&rec), sizeof(rec));
    }

    stats.n_vertices = n_v;
    stats.n_faces    = n_f;
    stats.ok         = f.good();
    return stats;
}
