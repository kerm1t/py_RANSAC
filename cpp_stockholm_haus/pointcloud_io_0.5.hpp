...
struct GridImageConfig {
    ...
    int   max_image_dim    = 2048; // cap so huge planes don't OOM
    bool  write_diff       = true; // also write kept/removed overlay
    bool  write_density    = true; // write raw density heatmap
};
...
save_grid_images(const std::string&            prefix,  // e.g. "debug/plane"
                 const std::vector<Point3f>&   pts,
                 ...
                 const GridImageConfig&        cfg = {})
{
    ...
    for (int pi = 0; pi < (int)planes.size(); ++pi) {
        ...
        // Build UV projections
        std::array<float,3> u, v;
        detail::plane_basis(pd.normal, u, v);

        std::vector<std::array<float,2>> uv(pd.inliers.size());
        float umin =  1e30f, vmin =  1e30f;
        float umax = -1e30f, vmax = -1e30f;
        for (size_t i = 0; i < pd.inliers.size(); ++i) {
            if (pd.inliers[i] >= (uint32_t)pts.size()) continue;
            const auto& p = pts[pd.inliers[i]];
            float pu = detail::dot3(p, u);
            float pv = detail::dot3(p, v);
            uv[i] = {pu, pv};
            umin=std::min(umin,pu); umax=std::max(umax,pu);
            vmin=std::min(vmin,pv); vmax=std::max(vmax,pv);
        }

        // Grid dimensions, capped at max_image_dim
        int nu_raw = std::max(1, (int)std::ceil((umax-umin) / cfg.cell_size));
        int nv_raw = std::max(1, (int)std::ceil((vmax-vmin) / cfg.cell_size));

        // Scale down if needed
        float scale = 1.f;
        if (nu_raw > cfg.max_image_dim || nv_raw > cfg.max_image_dim)
            scale = (float)cfg.max_image_dim / (float)std::max(nu_raw, nv_raw);

        int nu = std::max(1, (int)(nu_raw * scale));
        int nv = std::max(1, (int)(nv_raw * scale));

        if ((int64_t)nu * nv > 4'000'000) continue; // safety guard

        float eff_cell_u = (umax - umin) / nu;
        float eff_cell_v = (vmax - vmin) / nv;

        // Fill occupancy counts
        std::vector<int> counts(nu * nv, 0);
        for (auto& p2 : uv) {
            int ci = std::min((int)((p2[0]-umin)/eff_cell_u), nu-1);
            int cj = std::min((int)((p2[1]-vmin)/eff_cell_v), nv-1);
            counts[cj * nu + ci]++;
        }
        ...
        if (cfg.write_density) {
            std::vector<uint8_t> rgb(nu * nv * 3);
            for (int cj = 0; cj < nv; ++cj) {
                for (int ci = 0; ci < nu; ++ci) {
                    int cnt = counts[cj*nu+ci];
                    uint8_t r,g,b;
                    detail::density_color(cnt, max_count, r, g, b);
                    size_t off = ((size_t)(nv-1-cj)*nu + ci)*3; // flip V (image Y↓)
                    rgb[off]=r; rgb[off+1]=g; rgb[off+2]=b;
                }
            }
            std::string path = prefix + "_plane" + std::to_string(pi) + "_density.png";
            if (detail::write_png(path, nu, nv, rgb)) stats.n_images++;
        }

        // ── Diff image: kept=green, removed=red, empty=black ──────────
        if (cfg.write_diff) {
            std::vector<uint8_t> rgb(nu * nv * 3, 0);
            for (int cj = 0; cj < nv; ++cj) {
                for (int ci = 0; ci < nu; ++ci) {
                    int cnt = counts[cj*nu+ci];
                    if (cnt == 0) continue;
                    // brightness proportional to log density
                    float t = std::log((float)cnt+1.f) / std::log((float)max_count+1.f);
                    uint8_t bright = (uint8_t)(80 + 175*t);
                    size_t off = ((size_t)(nv-1-cj)*nu + ci)*3;
                    if (cnt >= cfg.min_pts_per_cell) {
                        // kept: green
                        rgb[off]=0; rgb[off+1]=bright; rgb[off+2]=0;
                    } else {
                        // removed: red
            ...
            std::string path = prefix + "_plane" + std::to_string(pi) + "_diff.png";
            if (detail::write_png(path, nu, nv, rgb)) stats.n_images++;
        }
    }
